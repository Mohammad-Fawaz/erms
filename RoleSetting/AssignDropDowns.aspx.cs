...
public partial class AssignDropDowns : System.Web.UI.Page
{
    public String _SID;
    ...
    public void BindGridview()
    {
        ...
        Int32 ddlPagesa = Convert.ToInt32(ddlPages.SelectedValue);
        int RoleID = Int32.Parse(AppSecProfile.SelectedValue.ToString());
        int securityMode = Int32.Parse(ddlSecurityMode.SelectedValue.ToString());
        using (DataTable _PagesGrid = DataAccess.GetRecords(DataQueries.GetAssigningFields(ddlPagesa, RoleID, securityMode)))
        {
    ...
    public void BinddropdownMainModules()
    {
        ...
        //_tempDispType.Rows.Add("0", "-Select Profile-");
        _tempDispType.Merge(DataAccess.GetRecords(DataQueries.GetAllPagesWherePageIdiSnOTnULLFiedwise()), true);
    ...
    public void BindPages()
    {
        ...
        _tempDispType.Merge(DataAccess.GetRecords(DataQueries.GetAllSubPages(Convert.ToInt32(ddl_Main_Modules.SelectedValue))), true);
        this.ddlPages.DataSource = _tempDispType;
        this.ddlPages.DataTextField = "PageName";
        this.ddlPages.DataValueField = "ID";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            int PageId = Int32.Parse(ddlPages.SelectedValue.ToString());
            int securityMode = Int32.Parse(ddlSecurityMode.SelectedValue.ToString());
            //  DataAccess.ModifyRecords(DataQueries.DeleteFromRolePages(RoleID));
            DataAccess.ModifyRecords(DataQueries.DeleteDropsDownAll(PageId, RoleID, securityMode));
            foreach (GridViewRow gr in GridView1.Rows)
            {
                ...
                HiddenField hfd_MasterFieldID = (HiddenField)gr.FindControl("hfd_MasterFieldID");
                HiddenField hfdSubFieldID = (HiddenField)gr.FindControl("hfdSubFieldID");
                // You can get other value same way               
                
                if (cb != null && cb.Checked)
                {
                    lblStatus.Text = "Succesfully Save Record";
                    lblStatus.ForeColor = System.Drawing.Color.Green;
                    DataAccess.ModifyRecords(DataQueries.Insert_Dropdowns(PageId, hfd_MasterFieldID.Value, hfdSubFieldID.Value, RoleID, securityMode));
                    //DataAccess.ModifyRecords(DataQueries.Insert_Dropdowns(PageId, hfd_MasterFieldID.Value, hfdSubFieldID.Value, RoleID, securityMode));
                }
                else
                {
                    DataAccess.ModifyRecords(DataQueries.DeleteDropdowns(PageId, hfd_MasterFieldID.Value, hfdSubFieldID.Value, RoleID, securityMode));
                }
            }
    ...
    protected void ddlSecurityMode_SelectedIndexChanged(object sender, EventArgs e)
    {
        BindGridview();
    }
}
